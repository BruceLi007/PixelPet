...
├── models/
│   └── pixel_pet.py           # 像素宠物模型（位置、方向、动画、跳跃物理）
├── views/
...
跳跃高度和重力可在 `models/pixel_pet.py` 中调整：

```python
self.jump_height = 120  # 跳跃目标高度（像素）
self.gravity = 2.0      # 重力加速度
```
...
| `jump/` | `jump-1.png` ~ `jump-4.png` | 原地跳跃动画 |
